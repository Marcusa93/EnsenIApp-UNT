...
export function useCardProgress({ studentId, recordingId, initial }: UseCardProgressOptions) {
  ...
  const record = React.useCallback(
    (index: number, outcome: { known: boolean; correct?: boolean }) => {
      setProgress((prev) => {
        const cur: CardProgressState = prev[index] ?? { known: false, attempts: 0, correct: 0 };
        const next: CardProgressState = {
          known: outcome.known,
          attempts: cur.attempts + 1,
          correct: cur.correct + (outcome.correct ?? outcome.known ? 1 : 0),
        };
        ...
        void persistCardProgress(row).finally(() => setPendingWrites((n) => Math.max(0, n - 1)));
  ...
  return { progress, record, pendingWrites };
}
