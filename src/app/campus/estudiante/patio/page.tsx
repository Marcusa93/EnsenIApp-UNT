...
export const metadata: Metadata = { title: "El Patio · EnsenIA UNT" };
...
export default async function PatioPage() {
  ...
  if (!course) {
    return (
      <>
        <PageHeader eyebrow="El Expediente" title="El Patio" />
        <EmptyState
          icon={GraduationCap}
    ...
    return (
      <>
        ...
        <EmptyState
          ...
          description="Al patio se entra con tu operador puesto. Se crea en un minuto desde Juegos."
          action={
            <Button asChild>
  ...
  return (
    <>
      <PageHeader
        ...
        title="El Patio"
        description="Quiénes andan por el patio y en qué mesa están. Acercate a una y jugá el desafío de esa clase; para saludar, usá los emotes."
      />
      ...
      {tables.length === 0 ? (
        <EmptyState
          ...
          description="Las mesas se arman con las clases que ya tienen desafíos generados. Cuando el equipo docente prepare la primera, el patio abre."
        />
      ) : (
        <LibraryRoom
          courseId={course.id}
